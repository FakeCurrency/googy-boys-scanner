...
- **THE FILTER (owner spec, 2026-09-08, revised same day): LONG only, never a
  ...
  high-conviction longs only -- a tight list (~7/day). `MORNING_PLAYS_INCLUDE_
  ALL_APLUS` (config, default False) widens it to also include every plain long
  A+ (~100/day, capped at `MORNING_PLAYS_MAX_ROWS`/market with a "+N more"
  line). Output is CLEAN TEXT, not embeds: `SYMBOL -> label` grouped under
  `**ASX plays**` / `**NASDAQ plays**`, where the label is "A+ High conviction"
  / "High conviction" / "A+". Messages are chunked to stay under Discord's
  2000-char `content` limit (`_chunk`), so multiple posts land if the list is
  long. No entry/stop/RR/company-name clutter -- the owner asked for a scannable
  ticker list.
