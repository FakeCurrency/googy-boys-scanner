...
def _read(label, sectors):
    """Plain-English market summary + rotation read from the sector moves."""
    if not sectors:
        return "", ""
    ...
    summary = (f"{label}: {len(up)} sectors up, {len(down)} down. "
               f"{best['name']} led ({best['chg_pct']:+.1f}%), {worst['name']} lagged "
               f"({worst['chg_pct']:+.1f}%). Tone reads "
               f"{'risk-on' if risk_on else 'risk-off / defensive'} — cyclicals "
               f"{cyc_avg:+.1f}% vs defensives {dfn_avg:+.1f}%.")
    leaders = ", ".join(s["symbol"] for s in ranked[:3])
    laggards = ", ".join(s["symbol"] for s in ranked[-3:][::-1])
...
def fetch() -> dict:
    ...
    for key, label, idx_rows, sec_rows in [
        ...
        summary, rotation = _read(label, sectors)
