...
NB: every public call routes through _retry() for exponential-backoff on
transient network/API errors. Do not add un-retried duplicates of these
functions — a previous version shadowed the retry-wrapped copies, silently
disabling retries on the live order path.
"""
