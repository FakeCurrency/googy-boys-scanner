...
- [x] 96. test/contrast.test.js parses the :root colour tokens and gates WCAG contrast on the surfaces the UI renders: text tokens ≥4.5:1, accent/UI-glyph colours ≥3:1 (33 pairs). Runs in CI (test.yml). Current palette passes with headroom (text ≥7:1, tightest accent blue-on-panel-2 at 3.94:1); a token dimmed below its bar fails the build.
- [x] 97. The SWR cache extracted to js/cache.js (window.GBSCache; app.js aliases it, behaviour byte-identical, defensive fallback). test/cache.test.js covers set/get, TTL expiry boundary, stale-payload paint, head-cache field-stripping + HEAD_ROWS cap, the 500KB journal-safety cap, and corrupt-JSON resilience — 13 tests, mocked localStorage + stubbed clock, in CI.
- [x] 98. test/e2e/screenshot-diff.e2e.js gates >2% visual drift at desktop + 390px (index + journal). Non-flaky by design: baselines self-capture per environment via GitHub Actions cache (never committed — cross-env font rendering can't false-fail), all time-based elements are masked + motion frozen, and pixelmatch ignores anti-aliasing so only structural drift counts. Verified 0.00% on an identical re-run and 27.74% (→ fail) on an injected regression. Diffs upload as a CI artifact; bump the cache key to accept an intentional change.
- [x] 99. version.json deploy stamp + js/telemetry.js (loaded first in <head> on every page): a window.onerror + unhandledrejection beacon (ring buffer + window.__gbsErrors()), plus skew detection that re-checks version.json on focus and offers a refresh when a new deploy lands mid-session (the SW controllerchange is the same signal). Verified it captures a synthetic uncaught error and reads the stamp.
- [x] 100. FRONTEND.md written — deck architecture, the breakpoint scale (with the 320/390 CI gates), the four cache layers, the recs three-source data flow, and the conventions (versioning, tokens, timestamps, a11y, telemetry, shared chrome) + the CI testing gates.
